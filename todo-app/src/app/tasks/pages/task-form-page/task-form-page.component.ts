...
export class TaskFormPageComponent implements OnInit {
  ...
  constructor(
      ...
      private getTaskHankdler: GetTaskHandler
  ) { }
    ...
    async loadTask(): Promise<void>{
      const response = await this.getTaskHankdler.execute(this.taskId || '')
      if(response){
    ...
    async onSubmit(): Promise<void>{
      const taskToSave: ITask = {
        ...this.form.value, //pagando todos os valores do formulário
